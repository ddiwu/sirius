...
__global__ void probe_right_semi_anti(T** keys,
    ...
    if (threadIdx.x + (ITEM * B) < num_tile_items) {
      ...
      if (equal_keys == 1)
        ...
        slot = hash64_multikey(keys[0][tile_offset + threadIdx.x + ITEM * B],
                               keys[1][tile_offset + threadIdx.x + ITEM * B]) %
               ht_len;
      else
        cudaAssert(0);
...
__global__ void scan_right(unsigned long long* ht,
                           ...
                           int is_count)
{
  ...
  if (blockIdx.x == num_tiles - 1) { num_tile_items = ht_len - tile_offset; }
  ...
  for (int ITEM = 0; ITEM < I; ITEM++) {
    if (threadIdx.x + (ITEM * B) < num_tile_items) {
      ...
      if (join_mode == 0) {  // semi join
        ...
        if (ht[slot * (num_keys + 2) + num_keys + 1] == 0xFFFFFFFFFFFFFFFF &&
            ht[slot * (num_keys + 2) + num_keys] != 0xFFFFFFFFFFFFFFFF) {
          items_off[ITEM]       = ht[slot * (num_keys + 2) + num_keys];
          selection_flags[ITEM] = 1;
          t_count++;
        }
...
template __global__ void scan_right<BLOCK_THREADS, ITEMS_PER_THREAD>(unsigned long long* ht,
                                                                     unsigned long long* count,
                                                                     uint64_t ht_len,
                                                                     uint64_t* row_ids,
                                                                     uint64_t num_keys,
                        ...
                        int num_keys)
{
  ...
  CHECK_ERROR();
  scan_right<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(ht_len + tile_items - 1) / tile_items, BLOCK_THREADS>>>(
      ht, (unsigned long long*)count, ht_len, row_ids, num_keys, join_mode, 1);
  CHECK_ERROR();
                                 ...
                                 int num_keys)
{
  ...
  int tile_items = BLOCK_THREADS * ITEMS_PER_THREAD;
  probe_right_semi_anti<BLOCK_THREADS, ITEMS_PER_THREAD, T>
...
template void probeHashTableRightSemiAnti<int32_t>(uint8_t** keys,
                                                   unsigned long long* ht,
                                                   uint64_t ht_len,
                                                   uint64_t N,
                                                   int* condition_mode,
...
template void probeHashTableRightSemiAnti<int64_t>(uint8_t** keys,
                                                   unsigned long long* ht,
                                                   uint64_t ht_len,
                                                   uint64_t N,
                                                   int* condition_mode,
