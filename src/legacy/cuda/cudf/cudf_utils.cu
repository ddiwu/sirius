...
int32_t* convertUInt64ToInt32(uint64_t* data, size_t N)
{
  ...
  convert_uint64_to_int32<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(data, output_dev, N);
  CHECK_ERROR();
  return output_dev;
}
...
uint64_t* convertInt32ToUInt64(int32_t* data, size_t N)
{
  ...
  convert_int32_to_uint64<BLOCK_THREADS, ITEMS_PER_THREAD>
    <<<(N + tile_items - 1) / tile_items, BLOCK_THREADS>>>(data, output_dev, N);
  CHECK_ERROR();
  return output_dev;
}
